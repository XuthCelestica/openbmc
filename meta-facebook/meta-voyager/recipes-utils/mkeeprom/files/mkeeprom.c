...
#include <getopt.h>
#include "mkeeprom.h"
struct eeprom_st *eeprom;
bool set_fbw_local_mac = 0;
void dump_eeprom(struct eeprom_st *eeprom) {
	printf("dump_eeprom\n");
...
int write_binary(char* out) {
	...
	offset = 0;
	//fwrite(eeprom, sizeof(struct eeprom_st), 1, fp);
	//fwrite(eeprom, FBW_EEPROM_SIZE, 1, fp);

    memset(raw,0xff,FBW_EEPROM_SIZE);

	memcpy(raw,eeprom,sizeof(struct eeprom_st));
	//dump_eeprom(eeprom);
...
int main(int argc, char **argv) {
	...
	eeprom = (struct eeprom_st *) malloc(sizeof(struct eeprom_st));
	memset(eeprom, "\0", sizeof(*eeprom));
	eeprom->fbw_crc8 = 0;
	printf("sizeof(struct eeprom_st)=%d\n",(int)sizeof(struct eeprom_st));
