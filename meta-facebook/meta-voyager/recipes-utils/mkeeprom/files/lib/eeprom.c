...
#define FBW_EEPROM_FILE "/sys/bus/i2c/devices/i2c-6/6-0051/eeprom"
#endif
...
static int fbw_parse_buffer(
    const uint8_t *buf, int len, struct eeprom_st *eeprom) {
  int rc = 0;
...
int eeprom_parse(const char *fn, struct eeprom_st *eeprom)
{
  int rc = 0;
  uint32_t len;
  FILE *fin;
