...
#define FAN_MAX 255
#define FAN_MIN 76
#define FAN_NORMAL_MIN_F2B 89
#define FAN_NORMAL_MAX_F2B FAN_MAX
#define FAN_NORMAL_MIN_B2F 76
#define FAN_NORMAL_MAX_B2F 178
...
struct line_policy fishbone32_f2b_normal = {
	.temp_hyst = CRITICAL_TEMP_HYST,
	.begin = {
		.temp = RAISING_TEMP_LOW,
		.speed = FAN_NORMAL_MIN_F2B,
	},
	.end = {
		.temp = RAISING_TEMP_HIGH,
		.speed = FAN_NORMAL_MAX_F2B,
	},
	.get_speed = calculate_line_speed,
};

struct line_policy fishbone32_f2b_onefail = {
	.temp_hyst = CRITICAL_TEMP_HYST,
	.begin = {
		.temp = RAISING_TEMP_LOW,
...
struct line_policy fishbone32_b2f_normal = {
	.temp_hyst = CRITICAL_TEMP_HYST,
	.begin = {
		.temp = RAISING_TEMP_LOW,
		.speed = FAN_NORMAL_MIN_B2F,
	},
	.end = {
		.temp = RAISING_TEMP_HIGH,
		.speed = FAN_NORMAL_MAX_B2F,
	},
	.get_speed = calculate_line_speed,
};

struct line_policy fishbone32_b2f_onefail = {
	.temp_hyst = CRITICAL_TEMP_HYST,
	.begin = {
		.temp = RAISING_TEMP_LOW,
		.speed = FAN_ONE_FAIL_MIN,
			...
			if(info->hwarn != BAD_TEMP &&
				...
				if(++info->warn_count >= ALARM_START_REPORT) {
					if(!(info->flag & HIGH_WARN_BIT))
						syslog(LOG_WARNING, "Major temp alarm: %s arrived %d C(High Warning: >= %d C), set fan max speed",
